...
addChapter("proba", {
  ...
  /* -------------------- DÉFINITIONS -------------------- */
  definitions: [
    { term: "Processus stochastique", def: String.raw`Famille de variables aléatoires $\{X_t,\ t\in I\}$ indexée par le temps. Le temps peut être discret ($I=\{0,1,2,\dots\}$) ou continu, et l'espace d'états discret ou continu.` },
    { term: "Espace d'états", def: String.raw`Ensemble $S$ des valeurs que peut prendre le processus. Dans ce chapitre, $S$ est dénombrable (souvent fini).` },
    { term: "Chaîne de Markov à temps discret", abbr: "DTMC", def: String.raw`<em>Discrete-time Markov chain</em> : suite $\{X_n,\ n\ge 0\}$ à valeurs dans $S$ vérifiant la propriété de Markov et l'homogénéité temporelle, entièrement décrite par sa distribution initiale $a$ et sa matrice de transition $P$.` },
    { term: "Propriété de Markov", def: String.raw`$P(X_{n+1}=j\mid X_n=i,X_{n-1},\dots,X_0)=P(X_{n+1}=j\mid X_n=i)$ : conditionnellement au présent, le futur ne dépend pas du passé. Si la description naturelle a de la mémoire, on <strong>agrandit l'état</strong> pour la retrouver.` },
    { term: "Homogénéité temporelle", def: String.raw`Les probabilités de transition $P_{ij}$ ne dépendent pas de l'instant $n$ : une seule matrice $P$ décrit toute l'évolution.` },
    { term: "Matrice de transition (stochastique)", def: String.raw`$P=(P_{ij})$ avec $P_{ij}=P(X_{n+1}=j\mid X_n=i)$. Coefficients positifs et <strong>chaque ligne somme à 1</strong>. Ligne $=$ état de départ, colonne $=$ état d'arrivée.` },
    { term: "Diagramme de transition", def: String.raw`Graphe orienté dont les sommets sont les états et les arcs les transitions de probabilité non nulle, étiquetés par $P_{ij}$. Sert à lire les classes, la période et les états absorbants.` },
    { term: "Distribution initiale", def: String.raw`Vecteur ligne $a=[P(X_0=0),P(X_0=1),\dots]$. La distribution à l'instant $n$ est $p_n=aP^n$ (vecteur ligne à gauche de la matrice).` },
    { term: "Probabilité de transition en n pas", def: String.raw`$P^{(n)}_{ij}=P(X_n=j\mid X_0=i)$, coefficient $(i,j)$ de la puissance $P^n$.` },
    { term: "Équations de Chapman-Kolmogorov", abbr: "C-K", def: String.raw`$P^{(m+n)}_{ij}=\sum_k P^{(m)}_{ik}P^{(n)}_{kj}$ : aller de $i$ à $j$ en $m+n$ pas, c'est passer par un état intermédiaire $k$ après $m$ pas. Matriciellement, $P^{(n)}=P^n$.` },
    { term: "État accessible, états qui communiquent", def: String.raw`$j$ est accessible depuis $i$ ($i\to j$) s'il existe $n\ge 0$ avec $P^{(n)}_{ij}>0$. Si $i\to j$ et $j\to i$, les deux états <strong>communiquent</strong> ($i\leftrightarrow j$).` },
    { term: "Classe de communication", def: String.raw`Ensemble maximal d'états qui communiquent tous entre eux. La relation « communiquer » partitionne $S$ en classes.` },
    { term: "Chaîne irréductible", def: String.raw`Chaîne qui ne possède qu'une seule classe de communication : tous les états communiquent.` },
    { term: "État récurrent / transitoire", def: String.raw`Récurrent si, partant de $i$, on y revient avec probabilité $f_i=1$ (visité infiniment souvent) ; transitoire si $f_i\lt 1$ (visité un nombre fini de fois). Critère : $i$ récurrent $\iff\sum_n P^{(n)}_{ii}=\infty$. Propriété de classe.` },
    { term: "Récurrence positive", def: String.raw`État récurrent dont le temps de retour moyen $m_i$ est fini. Dans une chaîne à espace d'états fini, tout état récurrent est récurrent positif.` },
    { term: "Classe fermée", def: String.raw`Classe dont aucune transition ne sort. Dans une chaîne finie, une classe est récurrente si et seulement si elle est fermée.` },
    { term: "État absorbant", def: String.raw`État $i$ tel que $P_{ii}=1$ : une fois atteint, on n'en sort plus. Il forme à lui seul une classe fermée.` },
    { term: "Période, apériodicité", def: String.raw`$d(i)=\gcd\{n\ge 1: P^{(n)}_{ii}>0\}$. L'état est apériodique si $d(i)=1$ ; une boucle $P_{ii}>0$ suffit. Propriété de classe.` },
    { term: "Chaîne ergodique", def: String.raw`Irréductible $+$ récurrente positive $+$ apériodique. Il existe alors une unique distribution stationnaire $\pi$ et $P(X_n=i)\to\pi_i$ quel que soit l'état de départ.` },
    { term: "Distribution stationnaire", def: String.raw`Vecteur $\pi$ solution de $\pi=\pi P$ avec $\sum_i\pi_i=1$. Unique si la chaîne est irréductible et récurrente positive. $\pi_i$ est la fraction de temps passée en $i$ à long terme.` },
    { term: "Équations de balance", def: String.raw`Les équations $\pi_j=\sum_i\pi_iP_{ij}$ du système $\pi=\pi P$. L'une d'elles est toujours redondante : on la remplace par la normalisation $\sum_i\pi_i=1$.` },
    { term: "Temps de retour moyen", def: String.raw`$m_i$ = espérance du nombre de pas pour revenir en $i$ en partant de $i$. Pour une chaîne irréductible récurrente positive, $m_i=1/\pi_i$.` },
    { term: "Distribution limite", def: String.raw`$\lim_{n\to\infty}P(X_n=i)$. Existe et vaut $\pi_i$ pour toute distribution initiale si la chaîne est ergodique ; une chaîne périodique a une distribution stationnaire mais pas de limite.` },
    { term: "Coût (ou récompense) moyen à long terme", def: String.raw`Si un coût $C(i)$ est payé à chaque passage en $i$ : $\lim\frac1n\sum_{k=1}^nC(X_k)=\sum_iC(i)\pi_i$. Moyenne temporelle sur une trajectoire $=$ moyenne d'ensemble pondérée par $\pi$ (ergodicité).` },
    { term: "PageRank", def: String.raw`Rang d'une page web $=$ sa fréquence de visite à long terme $\pi_j$ par un surfeur aléatoire qui suit à chaque étape un lien sortant choisi uniformément.` },
    { term: "Cul-de-sac (dead end)", def: String.raw`Page sans lien sortant : la ligne correspondante de $P$ est nulle, la masse de probabilité « fuit ». Casse l'ergodicité.` },
    { term: "Piège (spider trap)", def: String.raw`Groupe de pages dont on ne peut plus sortir (classe fermée) : il capte toute la masse de probabilité à long terme.` },
    { term: "Taxation (téléportation)", def: String.raw`Avec probabilité $\beta$ le surfeur suit un lien, avec probabilité $1-\beta$ il saute vers une page uniforme : $G=\beta P+\frac{1-\beta}{n}\mathbf 1\mathbf 1^\top$, matrice strictement positive donc ergodique.` },
    { term: "Itération de puissance", def: String.raw`Calcul de $\pi$ par $p_{k+1}=p_kG$ à partir d'un $p_0$ quelconque ; converge au taux $\beta$ : $\lVert p_k-\pi\rVert_1\le\beta^k\lVert p_0-\pi\rVert_1$.` },
  ],
});
