...
addChapter("fin-eng", {
  ...
  /* -------------------- DÉFINITIONS -------------------- */
  definitions: [
    { term: "Investissement", def: String.raw`Engagement présent d'argent ou de ressources dans l'espoir d'un bénéfice futur : consommation réduite aujourd'hui, consommation planifiée plus tard.` },
    { term: "Actif réel", def: String.raw`Actif qui sert directement à produire des biens et des services : terrain, bâtiment, machine, brevet, connaissance, capital humain.` },
    { term: "Actif financier", def: String.raw`Droit (créance) sur les revenus générés par des actifs réels ou par l'État : action, obligation, option, billet de banque. Ne contribue pas directement à la production.` },
    { term: "Marché financier", def: String.raw`Mécanisme permettant d'acheter et de vendre des titres à faible coût de transaction, à des prix reflétant l'information disponible. Trois fonctions : lever du capital, transférer le risque, faciliter le commerce international.` },
    { term: "Marché de capitaux", def: String.raw`Marché des titres à long terme et plus risqués : actions, obligations, dérivés.` },
    { term: "Marché monétaire", def: String.raw`Marché des titres de dette à court terme, liquides et peu risqués (quasi-liquidités) : bons du Trésor, CD, billets de trésorerie, repos.` },
    { term: "Marché primaire / secondaire", def: String.raw`Primaire : émission de nouveaux titres (l'émetteur reçoit les fonds, placés par les banques d'investissement). Secondaire : négociation de titres déjà émis entre investisseurs.` },
    { term: "Titre à revenu fixe", def: String.raw`Titre de dette promettant un flux de revenus fixe ou calculé selon une formule : obligation d'État ou d'entreprise, bon du Trésor.` },
    { term: "Action ordinaire", def: String.raw`Part de propriété de l'entreprise : une voix par action, part des bénéfices, <strong>créance résiduelle</strong> (dernier servi) et <strong>responsabilité limitée</strong> (on ne peut perdre que sa mise).` },
    ...
    { term: "Fonds indiciel coté", abbr: "ETF", def: String.raw`<em>Exchange-traded fund</em> : fonds répliquant un indice et négocié en bourse comme une action. Outil de la gestion passive.` },
    { term: "Option", def: String.raw`Contrat donnant le <em>droit</em> (et non l'obligation) d'acheter (<strong>call</strong>) ou de vendre (<strong>put</strong>) le sous-jacent à un prix fixé (<strong>strike</strong> $K$) jusqu'à une date fixée (<strong>échéance</strong> $T$), contre une <strong>prime</strong> payée d'avance.` },
    { term: "Payoff", def: String.raw`Ce que rapporte l'option à l'exercice : call $\max(S_T-K,0)$, put $\max(K-S_T,0)$. Le profit retranche la prime.` },
    { term: "Option européenne / américaine", def: String.raw`Européenne : exerçable seulement à l'échéance. Américaine : exerçable à tout moment avant l'échéance (il faut aussi décider <em>quand</em> exercer).` },
    { term: "Option dépendante du chemin", def: String.raw`<em>Path-dependent</em> : le payoff dépend de toute la trajectoire du prix. Barrière (naît ou meurt si un niveau est atteint), asiatique (moyenne du prix), lookback (maximum ou minimum).` },
    { term: "Vendeur d'option (writer)", def: String.raw`Celui qui vend l'option et encaisse la prime : il a une <em>obligation</em>, symétrique du droit de l'acheteur, d'où la question de la couverture de son risque.` },
    { term: "Obligation", def: String.raw`Contrat payé d'avance versant un montant connu (<strong>valeur faciale</strong>) à une date connue (<strong>maturité</strong>), plus éventuellement des <strong>coupons</strong>. <strong>Zéro-coupon</strong> : seulement la valeur faciale.` },
    { term: "Contrat forward", def: String.raw`Accord de gré à gré d'acheter ou vendre un actif à un prix fixé (prix forward) à une date fixée. Livraison obligatoire, rien payé à l'entrée, gain ou perte réalisé à l'échéance.` },
    { term: "Contrat futures", def: String.raw`Forward standardisé négocié en bourse, avec dépôt de garantie (<strong>appel de marge</strong>) et <strong>règlement quotidien</strong> des variations de valeur.` },
    { term: "Position longue / courte", def: String.raw`Longue : acheteur du contrat (gagne si le prix monte). Courte : vendeur (gagne si le prix baisse).` },
    { term: "Gré à gré", abbr: "OTC", def: String.raw`<em>Over-the-counter</em> : négociation directe entre deux parties, hors bourse. Forwards, swaps et dérivés de taux s'y échangent surtout.` },
    { term: "Swap de taux", abbr: "IRS", def: String.raw`<em>Interest rate swap</em> : échange de flux d'intérêts à taux fixe contre des flux à taux variable sur un montant notionnel. Principal dérivé de taux d'intérêt.` },
    { term: "Contrepartie centrale", abbr: "CCP", def: String.raw`<em>Central counterparty</em> : chambre de compensation qui s'interpose entre acheteur et vendeur pour réduire le risque de défaut ; imposée après 2008 pour des produits OTC standardisés comme les CDS (<em>credit default swaps</em>).` },
    { term: "Hedger, spéculateur, arbitragiste", def: String.raw`Hedger : réduit un risque existant. Spéculateur : parie sur la direction d'une variable de marché. Arbitragiste : positions compensées sur plusieurs instruments pour un profit sans risque.` },
    { term: "Arbitrage", def: String.raw`Profit certain sans mise de fonds ni risque, obtenu en exploitant un écart de prix. Dans un marché concurrentiel, ces opportunités disparaissent rapidement.` },
    { term: "Valeur actuelle (actualisation)", def: String.raw`Montant à placer aujourd'hui pour recevoir $E$ avec certitude en $T$. À taux continu constant $r$ : $M(t)=E\,e^{-r(T-t)}$ ; à taux déterministe $r(\cdot)$ : $E\exp(-\int_t^Tr)$.` },
    { term: "Capitalisation continue / discrète", def: String.raw`Continue : $M(t)=Ce^{rt}$, solution de $dM/M=r\,dt$. Discrète $m$ fois par an : $(1+r/m)^{mT}$, dont la limite quand $m\to\infty$ est $e^{rT}$.` },
    { term: "Écart TED", abbr: "TED spread", def: String.raw`Écart entre le taux interbancaire (LIBOR) et le taux des bons du Trésor. Mesure de la tension et du risque de crédit dans le système bancaire (le cours l'écrit dans l'autre sens).` },
    { term: "Assouplissement quantitatif", abbr: "QE", def: String.raw`<em>Quantitative easing</em> : injection de capital et achats massifs d'actifs par la banque centrale pour soutenir les banques et le crédit, utilisé après la crise de 2008.` },
    { term: "Securities and Exchange Commission", abbr: "SEC", def: String.raw`Régulateur américain des marchés financiers. Sa règle 10b-5 encadre les transactions des dirigeants et actionnaires importants (délit d'initié).` },
  ],
});
